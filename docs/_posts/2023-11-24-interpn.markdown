...
<head>
    <svg viewBox="0 0 1200 800" width=600 height="auto">
        {% include 2023-11-24/with_prealloc.svg %}
    </svg>
</head>
...
allocation. The benchmarks are interleaved (method 1 x100, method 2 x100, method 3 x100, ...), so they see some performance cross-talk through allocation.

Eliminating allocation from the `interpn` methods _significantly improves scipy's performance_ in neighboring benchmarks, despite both methods using an un-instrumented
